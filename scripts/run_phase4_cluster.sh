...
LOG_DIR="${PHASE4_LOG_DIR:-$ROOT/.phase4-logs}"
...
for port in 18000 18001 18002; do
  if ss -ltn "sport = :$port" | grep -q LISTEN; then
    echo "port $port is already in use; refusing to start Phase-4 cluster" >&2
    exit 1
  fi
done
...
start_node() {
  ...
  env \
    ...
    CRDT_ANTI_ENTROPY_INTERVAL_SECONDS="${CRDT_ANTI_ENTROPY_INTERVAL_SECONDS:-2.0}" \
...
echo "Phase-4 cluster running. Logs: $LOG_DIR"
